...
def audit_route_plan(
    ...
    for r_name in unique_routes:
        ...
        v_start_min = parse_time_to_minutes(v_conf.get("horario_inicio", v_conf.get("start_time", "08:00")), 480)
        v_end_min = parse_time_to_minutes(v_conf.get("horario_fim", v_conf.get("end_time", "18:00")), 1080)
        v_max_kg = float(v_conf.get("capacidade_kg", v_conf.get("capacity", 1000.0)) or 1000.0)
        v_max_vol = float(v_conf.get("capacidade_volume", v_conf.get("capacidade_vol", v_conf.get("capacity_volume", 10.0))) or 10.0)
