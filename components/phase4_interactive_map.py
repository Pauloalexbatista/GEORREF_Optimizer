...
class Phase4InteractiveMap:
    ...
    def render():
        state = get_state()
        routes_df = state.routes_solution
        warehouses_df = state.warehouses_used
        ...
        with col_sync1:
            if st.button("🛰️ 1. Abrir Mapa Noutro Ecrã", use_container_width=True, help="Abre esta mesma interface numa janela nova para arrastares para o 2º monitor"):
                from utils.persistence_manager import create_snapshot
                snap_id = create_snapshot(state.projeto_atual, (state.utilizador_id or 1), 5, "SYNC_TRANSFER")
        ...
        with col_sync2:
            if st.button("💾 2. Enviar Edições para Principal", use_container_width=True, type="primary", help="Clica aqui no teu 2º monitor após fazeres edições"):
                from utils.persistence_manager import create_snapshot
                create_snapshot(state.projeto_atual, (state.utilizador_id or 1), 5, "SYNC_BACK")
        ...
        with col_sync3:
            if st.button("📥 3. Receber Edições do 2º Ecrã", use_container_width=True, help="Clica aqui na tua janela principal para atualizar o sistema"):
                ...
                snaps = get_snapshots_for_project(state.projeto_atual, limit=20)
